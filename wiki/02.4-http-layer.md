...
The library supports **Basic** (Username/Password), **Bearer** (Token-based), and **FMID** (FileMaker Cloud / Claris ID) authentication. Because FileMaker Server's OData implementation primarily relies on HTTP Basic auth, the layer provides helpers to ensure correct encoding and scheme detection.
...
### fmidAuth (v0.2.0)
The `fmidAuth` helper builds an `FMID <token>` authorization header for FileMaker Cloud / Claris ID authentication [src/http.ts:24-26]().

```typescript
import { fmidAuth } from 'fm-odata-js'
const token = fmidAuth(clarisIdToken) // 'FMID <token>'
```
...
5.  **Error Normalization**: If the response is not `ok`, it passes the response to `parseErrorResponse` to generate a structured `FMODataError` [src/http.ts:141-143]().
...
```mermaid
sequenceDiagram
    participant C as FMOData Client
...
| `resolveAuthHeader` | Ensures correct `Basic`, `Bearer`, or `FMID` formatting. |
...
The HTTP layer relies on the `HttpClientContext` which is typically managed by the `FMOData` client and passed down to every request.
