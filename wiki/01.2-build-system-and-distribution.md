...
This page documents the build pipeline and distribution strategy for `fm-odata-js`. The project uses a decoupled build process where TypeScript (`tsc`) handles type definition generation while `esbuild` manages the bundling and minification of the ESM artifacts.
...
2.  **ESM Bundling**: `esbuild` bundles the source into a single `fm-odata.esm.js` file, targeting `es2020` [package.json:26-26]().
3.  **Minification**: A secondary `esbuild` pass produces `fm-odata.esm.min.js`, applying minification for production use in environments like the FileMaker Web Viewer [package.json:27-27]().
4.  **IIFE Bundle**: A third `esbuild` pass produces `fm-odata.iife.min.js`, a minified IIFE bundle exposing a global `FMODataLib` for use in FileMaker Web Viewers and `<script>` tag inclusion without a bundler [package.json:28-28]().
...
graph TD
  ...
  subgraph "Distribution Artifacts (dist/)"
    ...
    ESM["fm-odata.esm.js (Standard ESM)"]
    MIN["fm-odata.esm.min.js (Minified ESM)"]
    IIFE["fm-odata.iife.min.js (IIFE Bundle)"]
  end
...
| :--- | :--- | :--- |
| **ESM Module** | `dist/fm-odata.esm.js` | Main entry point for modern bundlers and Node.js [package.json:6-7](). |
| **Minified ESM** | `dist/fm-odata.esm.min.js` | Optimized version for Web Viewers to reduce memory overhead (~9.1 KB gzipped) [package.json:27-27](). |
| **IIFE Bundle** | `dist/fm-odata.iife.min.js` | Minified IIFE bundle exposing global `FMODataLib` for `<script>` tag inclusion without a bundler [package.json:28-28](). |
...
This hint informs bundlers (like Webpack, Rollup, or Vite) that none of the modules in `fm-odata-js` execute code simply by being imported. If a consumer only imports the `Filter` class but not the `FMOData` client, the bundler can safely remove the client-related code from the final production bundle.
...
Because `fm-odata-js` is frequently used in FileMaker Web Viewers where script injection limits may apply, bundle size is a first-class metric.

### The `size` Script
The project includes a dedicated `size` utility [package.json:35-35](). This script:
1.  Takes the minified artifact (`dist/fm-odata.esm.min.js`).
