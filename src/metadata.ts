 ...
 * 1b. Same as 1a but with reversed attribute order
 * 1c. ServerVersion annotation (FM 26+, e.g. "OData Engine 26.0.1")
 * 1d. Same as 1c but with reversed attribute order
 * 2.  Generic fallback: any annotation term containing "Version" with major >= 17
 *
 * Returns the full FMServerVersion object, or undefined if not found.
 */
function extractServerVersion(xml: string): FMServerVersion | undefined {
  return parseServerVersion(xml) ?? undefined
}
...
export function parseMetadata(xml: string): ODataMetadata {
  try {
    ...
    if (serverVersion) {
      ...
      // Keep productVersion for backward compatibility (raw string only)
